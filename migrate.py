...
class GitLabMigration:
    ...
    def migrate(self, src_group_id, dest_group_id):
        self.__migrate_groups(src_group_id, dest_group_id)
        print("migration finished.")

    def __migrate_groups(self, src_group_id, dest_group_id):
        """
        Migrate a group
        """
        src_group = self.gl_src.groups.get(src_group_id)
        src_dict = self.__get_subgroups_by_name(src_group)

        dest_group = self.gl_dest.groups.get(dest_group_id)
        dest_dict = self.__get_subgroups_by_name(dest_group)
        ...
        for subgroup_name, src_subgroup in src_dict.items():
            ...
            if dest_subgroup is None:
                ...
                dest_subgroup = self.__create_subgroup(dest_group, subgroup_name)

            self.__migrate_groups(src_subgroup.id, dest_subgroup.id)

        # migrate projects
        src_project_dict = self.__get_projects_by_name(src_group)
        dest_project_dict = self.__get_projects_by_name(dest_group)
        for project_name, src_project in src_project_dict.items():
            ...
            if dest_project is None:
                ...
                dest_project = self.__create_project(dest_group, project_name)

            self.__migrate_project(src_project, dest_project)

    def __create_subgroup(self, group, name):
        """
    ...
    def __create_project(self, group, name):
        """
    ...
    def __migrate_project(self, src_project, dest_project):
        """
        Migrate a project
        """
        def get_url(gl, p):
    ...
    @staticmethod
    def __get_subgroups_by_name(group):
        result = dict()
    ...
    @staticmethod
    def __get_projects_by_name(group):
        result = dict()
